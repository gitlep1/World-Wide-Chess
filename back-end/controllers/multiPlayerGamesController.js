...
games.get("/", requireAuth(), scopeAuth(["read:user"]), async (req, res) => {
  const allGames = await getAllMultiGames();
...
games.get(
  ...
  async (req, res) => {
    ...
    const gotAGame = await getMultiGameByID(id);
...
games.post(
  ...
  async (req, res) => {
    ...
    const newGame = await createMultiGame(newGameData);
...
games.put(
  ...
  async (req, res) => {
    ...
    const updateGameData = await updateMultiGame(id, updatedGameData);
...
games.put(
  ...
  async (req, res) => {
    ...
    const game = await getMultiGameByID(id);
    ...
    if (promotion !== "") {
      ...
      if (move) {
        ...
        const updatedGame = await updateMultiGame(id, updatedGameData);
...
games.delete(
  ...
  async (req, res) => {
    const { id } = req.params;
    const gameDelete = await deleteMultiGame(id);
